/* Helpers */
import { clone, keys, map, reduce, secure, shell, shuffle, sort } from
...
describe('Sorters', () => {
	...
	+ ' ascending order', () => {
		...
		test('randomized test', () => {
			retry(() => {
				...
				expect(sort(randomCollection, ascending))
	...
	+ ' of the given collection', () => {
		...
		test('randomized test', () => {
			retry(() => {
				const shuffled = shuffle(getRndCollection());
	...
	+ ' on a given property', () => {
		...
		test('randomized test', () => {
			retry(() => {
				...
				const neededColl = secure(map(collection, (result) =>
					map(result, () => rndBetween(-10, 10))));
				const rndProp = rndKey(rndValue(collection));
				const sorter = rndValue([ascending, descending]);
				let count = 0;

				const expected = map(neededColl, (result) => {
