...
import { contains, filter, shares, map, values,
	...
	clean, select, keys } from '@laufire/utils/collection';
...
import {
	...
	rndArray, randomValues, arrayOrObject,
} from '../test/helpers';
...
describe('Predicates', () => {
	...
	+ ' wholly contained in any of the elements', () => {
		test('example', () => {
			...
			expect(find(tasks, isPart(taskToFind))).toEqual(taskToFind);
		});
		...
		test('randomized test', () => {
			retry(() => {
				...
				const needle = rndValue(haystack);
				const partialData = rndValue(needle);

				expect(find(haystack, isPart(needle))).toEqual(needle);
				expect(find(haystack, isPart(partialData))).toEqual(undefined);
			});
		});
	});
	...
	+ ' any of the elements', () => {
		...
		test.skip('example', () => {
			...
			const needle = ['RURABLMK', 'NAMDRIPB', 'ROZAOUTG', 'ZTFIZZVS'];
			const expectation = [
				'PIMEQKEC',
				'RURABLMK',
				'NAMDRIPB',
			...
			expect(find(data, doesContain(needle))).toEqual(expectation);
		});

		test.skip('randomized test', () => {
			retry(() => {
				const haystack = rndCollections(2, 3);
				const expectation = rndValue(haystack);
				const needle = randomValues(expectation);
				const error = [];

				try {
					const result = find(haystack, doesContain(needle));

					expect(result).toEqual(expectation);
				}

				catch {
					error.push({ haystack, needle, expectation });
	...
	+ ' collection functions', () => {
		test('example', () => {
			...
			expect(find(tasks, predicate(isEqual, taskToFind)))
				.toEqual(taskToFind);
		});
		...
		test('randomized test', () => {
			retry(() => {
				...
				expect(find(haystack, predicate(isEqual, needle)))
					.toEqual(needle);
			});
		});
	});
	...
	+ ' to the given predicate', () => {
		test('example', () => {
			const contact = {
				name: 'laufire',
				email: 'laufire@laufire.in',
			};

			expect(find(contact, key(isEqual('name')))).toEqual('laufire');
		});
	...
	+ ' to the given predicate', () => {
		test('example', () => {
			...
			expect(find(contact, value(isEqual('laufire')))).toEqual('laufire');
		});
